<h3 class="text-center">Rincian Nilai – Rekomendasi Pertanggungjawaban Uang Muka</h3><table class="table table-bordered"  id="table-rincian-nonsetoranumum">
            <thead>
                <tr class="inverse">
            ...
            @foreach($rincian as $k=>$v)
                <tr>
                    ...
                    <td class="text-center">{{rupiah($v->jumlah_um)}}</td>
                    <td class="text-center">
                        @if (Auth::user()->level == 'pic-unit')
                        <a href="javascript:addtindaklanjutrincian({{$v->id}},'nonsetoranpertanggungjawabanuangmuka','{{Config::get('constants.rincian.nonsetoranpertanggungjawabanuangmuka')}}')" class="btn-delete btn btn-xs btn-info"><i  class="glyphicon glyphicon-plus"></i></a>&nbsp;
                        @else
                            {{--<div class="btn-group">
                                                    ...
                                                    @else
                                                        @foreach($rinciantindaklanjut as $rtl)
                                                            @if($rtl->rekomendasi == $rekom->rekomendasi)
                                                                ...
                                                                @php $totalbtl = $v->jumlah_um - $rtl->sum; @endphp
                                                            @elseif($rekom->id == 3)
                        ...
                        @endif
                        <a href="javascript:listtindaklanjutrincian({{$v->id}},'nonsetoranpertanggungjawabanuangmuka')" class="btn-edit btn btn-xs btn-success"><i class="glyphicon glyphicon-list"></i></a>
                    </td>
                </tr>
                @php
                    $no++;
            ...
            @if (Auth::user()->level != 'pic-unit')
                    <tr >
                        <td class="text-center" colspan="8"><a href="#" onclick="addtindaklanjut('nonsetoranpertanggungjawabanuangmuka','{{$idtemuan}}','{{$idrekomendasi}}',-1)" class="label label-info" id="tombol-add-rincian" style="display:inline"><i class="fa fa-plus-circle"></i> Tambah Rincian</a></td>
                    </tr>
            @endif
